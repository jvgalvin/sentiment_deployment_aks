...
# resource "azurerm_postgresql_active_directory_administrator" "admins" {
#   server_name         = azurerm_postgresql_flexible_server.datacollector.name
...
resource "azurerm_postgresql_flexible_server" "datacollector" {
  ...
  resource_group_name    = azurerm_resource_group.rg.name
  location               = "eastus" // Why do this to me Azure?
  version                = "15"
